...
try {
  ...
  test('عقود واجهة 3.0 تمنع مربعات Windows مع استثناء مستكشف مجلد البيانات وتفصل حزمتَي x64 وx86 وتحفظ نص الملصق كاملًا', () => {
    ...
    const nativeDialogPattern = /showSaveDialog|showOpenDialog|window\.print\(|<input[^>]+type=["']file/i;
    assert.ok(!nativeDialogPattern.test([mainSource, preloadSource, viewsSource, settingsSource].join('\n')));
    assert.ok(mainSource.includes("ipcMain.handle('fs:openDataFolder'"));
    assert.ok(mainSource.includes('shell.openPath(folderPath)'));
    assert.ok(preloadSource.includes("openDataFolder: () => invoke('fs:openDataFolder')"));
    assert.ok(fileDialogSource.includes('window.raff.openDataFolder()'));
    ...
    assert.ok(v3Css.includes('line-height: 1.55 !important'));
    assert.ok(v3Css.includes('height: 46px !important'));
